...
setTimeout(function() {
 var resp = "Ok Jayesh following is the link for the Architecture of IBE<br>https://wiki.etraveli.net/display/Documentation/Architecture"
 ...
 if(text=='Whom should I contact for Payout Related Issue?'){
  var delayInMilliseconds = 1000; //1 second

setTimeout(function() {
 var resp = "You can contact David he is TL for IPA 2 Team"
     appendMessage(BOT_NAME, BOT_IMG, "left",resp);
     return;
}, delayInMilliseconds);
  ...
  if(text=='I need to know about Instant Payout'){
  var delayInMilliseconds = 1000; //1 second

setTimeout(function() {
 var resp = "Regular refund flow: ETG has a process/policy to do a refund in the following sequence: 1. Create case 2. Claim from carrier 3. Wait for carrier to refund to us 4. Do a payout to customer 5. Do the accounting step<>br Background for instant payout: <br> Due to the long waiting times for us to wait for carriers, and increased backlogs that causes frustration for customers, ETG is now changing this approach to be faster in certain scenarios. <br>For some predefined scenarios we enable a payout to be made BEFORE we have received the money from the provider. This will be valid for a carrier / order cancel reason-combination. The supported carriers are listed in the prop: Refund.InstantPayout.EnabledCarriers and supported cancel reasons are enabled using OrderCancelReason-config in Edvin."
     appendMessage(BOT_NAME, BOT_IMG, "left",resp);
     return;
}, delayInMilliseconds);
   ...
   if(text=='Some Events at Etraveli'){
  var delayInMilliseconds = 1000; //1 second

setTimeout(function() {
 var resp = "<iframe width='410' height='315' src='https://www.youtube.com/embed/qEPFQuHp_D4' title='YouTube video player' frameborder='0' allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture' allowfullscreen></iframe>"
     appendMessage(BOT_NAME, BOT_IMG, "left",resp);
     return;
}, delayInMilliseconds);
 ...
 var serverurl =  baseUrl+"/getResponseFromOpenAi";
...
/*fetch(serverurl, { 
  ...
  // alert(JSON.stringify(reqObj));
     ...
     error: function (jqXHR, textStatus, err) {
      //alert('text status '+textStatus+', err '+err.message)
     },
    });
}
...
function initChat(){
 ...
 // alert("Param value == "+paramval);
  hotelId = 'MfGqJsRJ7YZTbAt2YFNC';
  paramval = 'Ap0n1rQQ2ZEYemo1obVw';
 // alert(hotelId);
...
function WelcomeTxt(){
    var msg ="Hi Jayesh, Welcome to Etraveli Group I am your Buddy and  will be available for you all the time.you can ask anything or you can use suggestions below.";
